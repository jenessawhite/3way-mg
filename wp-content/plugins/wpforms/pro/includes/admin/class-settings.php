		...
		if ( ! empty( $_POST['wpforms-settings-general-nonce'] ) ) {
			...
			if ( ! wp_verify_nonce( $_POST['wpforms-settings-general-nonce'], 'wpforms-settings-general-nonce' ) || ! current_user_can( apply_filters( 'wpforms_manage_cap', 'manage_options' ) ) ) {
				...
				if ( isset( $_POST['submit-general'] ) ) {
					...
					$this->options['validation-file24h']         = ! empty( $_POST['validation-file24h'] ) ? sanitize_text_field( $_POST['validation-file24h'] ) : '';
			...
			<form method="post">
				...
				<table class="form-table">
					<tbody>
						...
						<tr>
							<th scope="row">
								<label for="wpforms-settings-general-validation-time24h"><?php _e( 'Time (12 hour)', 'wpforms' ); ?></label>
							</th>
							<td>
