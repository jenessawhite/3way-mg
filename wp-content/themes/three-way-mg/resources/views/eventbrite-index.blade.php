  ...
  <div class="row">
      ...
      @if( class_exists('Eventbrite_Query') )
        @if ( $the_query->have_posts() )
          @while ( $the_query->have_posts() )
            ...
            @endphp
            <article class="my-4" data-eventTime='{{$date}}'>
              <header class="graphic-header">
                <div class="row no-gutters">
                  <div class="eventbrite-event-photo">
                    <img src="{{the_field('event_featured_image')}}" alt="">
                  </div>
                  <div class="col bg-blue px-4 py-3">
                    <div class="row">
                      <div class="col-sm-12">
                        <div class="entry-meta">
                          <span class="event-time"> {{the_field('event_date')}}, {{the_field('event_start_time')}} - {{ the_field('event_end_time') }} </span>
                        </div>
                      </div>
                      <div class="col-sm-12">
                        <h2>{{the_title()}}</h2>
                      ...
                      <a href="{{the_permalink()}}">See More</a>
                    </div>
                  </div>
                </div>
              </header>
    ...
    <?php
      ...
      if ( $events->have_posts() ) :
        ...
        ?>
        <article id="event-<?php the_ID(); ?>" <?php post_class("tmg-event"); ?> data-eventTime="{{$eventbrite_date}}">
          <header class="graphic-header">
            <div class="row no-gutters">
